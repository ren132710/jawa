...
describe('#renderPageWeather', () => {
  ...
  const testPrefs = [
    { units: 'imperial', theme: 'jawa', lang: 'en' },
  ];
  ...
  before(function () {
    setTestDefaults();
    // deep equality
    expect(JSON.parse(localStorage.getItem('jawa-places'))).to.eql(
      testPlace,
    );
    expect(JSON.parse(localStorage.getItem('jawa-prefs'))).to.eql(
      testPrefs,
    );
  ...
  it('should pass smoke test of page layout', function () {
    ...
    cy.get('[data-place-search]')
      .invoke('attr', 'placeholder')
      .should('eq', 'Weather at your places');
    cy.get('[data-place-search]').should(
      'have.attr',
      'placeholder',
    ...
    cy.get('.current-top-left>[data-current-location]').should(
      'have.text',
      'new york',
    );
    //daily section
    ...
    cy.get('[data-hour-timezone]').should(
      'have.text',
      'America/New_York',
    );
  ...
  it('should correctly display current weather', function () {
    ...
    cy.get('[data-current-description]').should(
      'have.text',
      'clear sky',
    );
  ...
  it('should correctly display daily weather', function () {
    ...
    cy.get('@day1')
      .find('[data-daily-date]')
      .should('have.text', 'Tuesday');
    cy.get('@day1')
      .find('[data-daily-high]')
      .should('have.text', '75');
    cy.get('@day1')
      .find('[data-daily-low]')
      .should('have.text', '64');
    cy.get('@day1')
      .find('[data-daily-description]')
      .should('have.text', 'overcast clouds');
    cy.get('@day1')
      .find('[data-daily-humidity]')
      .should('have.text', '49');
    cy.get('@day1')
      .find('[data-daily-wind-speed]')
      .should('have.text', '21');
    cy.get('@day1')
      .find('[data-daily-wind-direction]')
      .should('have.text', 'S');
    cy.get('@day1')
    ...
    cy.get('.hourly-container')
      ...
      .then(() => {
        cy.get('[data-hour-date]').should(
          'have.text',
          'MondayMondayMondayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayTuesdayWednesdayWednesdayWednesday',
        );
        cy.get('[data-hour]').should(
          'have.text',
          '6PM8PM10PM12AM2AM4AM6AM8AM10AM12PM2PM4PM6PM8PM10PM12AM2AM4AM',
        );
        cy.get('[data-hour-temp]').should(
          'have.text',
          '777471686665646772757574727069686767',
        );
        cy.get('[data-hour-precip]').should(
          'have.text',
          '000000000000003268784',
        );
        cy.get('[data-hour-wind-speed]').should(
          'have.text',
          '141312109871112162021201716141210',
        );
        cy.get('[data-hour-wind-direction]').should(
          'have.text',
          'SSSSWSWSWSWSWSSSSSSSSSS',
        );
        cy.get('[data-hour-humidity]').should(
          'have.text',
          '384144464956636151495155647276849294',
        );
